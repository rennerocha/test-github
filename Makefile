...
build: ./backend/Dockerfile backend/requirements/production.txt ./frontend/Dockerfile ## Create the build and runtime images
	@docker build -t my_awesome_sixie_project_local_django:$(TAG) ./backend
	
	@docker build -t my_awesome_sixie_project_local_frontend:$(TAG) ./frontend
...
destroy-data: ## Remove the database volumes and start with a clean database
	...
	@docker volume rm my_awesome_sixie_project_local_postgres_data
	@docker volume rm my_awesome_sixie_project_local_postgres_data_backups
	@docker-compose up -d db


clean: ## Remove the latest build
	@docker rmi -f my_awesome_sixie_project_local_django:$(TAG)
...
ifeq ($(RELEASE_TAG), unspecified)
	...
	@aws --profile=my_awesome_sixie_project-deploy ecr batch-get-image --repository-name my_awesome_sixie_project --image-ids imageTag=$(RELEASE_TAG) \
		| jq '.images | length'| awk '$$1 != 1 {print "RELEASE_TAG does not match any ECR images"; exit 1 }'
	@aws --profile=my_awesome_sixie_project-deploy codepipeline get-pipeline --name my_awesome_sixie_project-staging \
		| jq 'del(.metadata) | .pipeline.stages[0].actions[0].configuration.ImageTag = "$(RELEASE_TAG)"' \
		| xargs -0 aws --profile=my_awesome_sixie_project-deploy codepipeline update-pipeline --cli-input-json 1>/dev/null
	@aws --profile=my_awesome_sixie_project-deploy codepipeline start-pipeline-execution --name my_awesome_sixie_project-staging
...
ifeq ($(RELEASE_TAG), unspecified)
	...
	@aws --profile=my_awesome_sixie_project-deploy ecr batch-get-image --repository-name my_awesome_sixie_project --image-ids imageTag=$(RELEASE_TAG) \
		| jq '.images | length'| awk '$$1 != 1 {print "RELEASE_TAG does not match any ECR images"; exit 1 }'
	@aws --profile=my_awesome_sixie_project-deploy codepipeline get-pipeline --name my_awesome_sixie_project-prod \
		| jq 'del(.metadata) | .pipeline.stages[0].actions[0].configuration.ImageTag = "$(RELEASE_TAG)"' \
		| xargs -0 aws --profile=my_awesome_sixie_project-deploy codepipeline update-pipeline --cli-input-json 1>/dev/null
	@aws --profile=my_awesome_sixie_project-deploy codepipeline start-pipeline-execution --name my_awesome_sixie_project-prod

check-x-release = @aws --profile=my_awesome_sixie_project-deploy codepipeline get-pipeline-state --name my_awesome_sixie_project-$(subst check-,,$(subst -release,,$@))  | jq '.stageStates[0].latestExecution.pipelineExecutionId as $$execId | { Pipeline: .pipelineName, Started: .stageStates[0].actionStates[0].latestExecution.lastStatusChange, Actions: .stageStates | map( .latestExecution.pipelineExecutionId as $$stage_execId | .actionStates | map( { (.actionName): (if $$stage_execId == $$execId then .latestExecution.status else "Pending" end) } ))  | flatten | add }'
...
build-dev: ./backend/Dockerfile backend/requirements/local.txt ./frontend/Dockerfile ## Create the dev build and runtime images
	@docker build --build-arg DEVEL=yes --build-arg TEST=yes -t my_awesome_sixie_project_local_django:dev ./backend
	
	@docker build -t my_awesome_sixie_project_local_frontend:$(TAG) ./frontend
	

outdated: ## Show all the outdated packages with their latest versions in the container
	@docker run --rm my_awesome_sixie_project_local_django:$(TAG) pip list --outdated

pipdeptree: ## Show the dependencies of installed packages as a tree structure
	@docker run --rm my_awesome_sixie_project_local_django:$(TAG) pipdeptree
...
load-dump: destroy-data ## Fresh load the data file passed in
	@sleep 5
	pg_restore -h localhost -p $(PGPORT) -j 4 -n public --no-owner -U my_awesome_sixie_project -W -d my_awesome_sixie_project $(DUMPFILE)

clean-dev: ## Force removal of the latest image
	@docker rmi -f my_awesome_sixie_project_local_django:dev
...
sandbox-secrets: ## Substitute with secrets template with env variable and run kubeseal
	...
	sed -i -e 's/namespace: my-awesome-sixie-project/namespace: my-awesome-sixie-project-sandbox/' -e'/spec:/ {N; s/spec:\n/  annotations:\n    sealedsecrets.bitnami.com\/managed: "true"\nspec:\n/}' k8s/sandbox/secrets.yaml
